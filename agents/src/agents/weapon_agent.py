...
SEED = os.getenv("WEAPON_AGENT_SEED", NAME)
PORT = int(os.getenv("WEAPON_AGENT_PORT", os.getenv("PORT", os.getenv("AGENT_PORT", "8101"))))
...
async def on_chat(ctx: Context, sender: str, msg: ChatMessage):
    ...
    await ctx.send(sender, ChatMessage(
        ...
        content=[TextContent(type="text", text=f"[weapon] pong: {text or 'ping'}")]
    ))
...
async def on_clip(ctx: Context, sender: str, msg: ClipReady):
    ...
    await ctx.send(sender, FrameScored(
        frame_url=msg.clip_url, score=score, label="weapon",
