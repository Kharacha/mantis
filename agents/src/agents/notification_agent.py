...
from src.common.schemas import HealthStatus, Alert

NAME = "notification"
SEED = os.getenv("NOTIFY_AGENT_SEED", NAME)
PORT = int(os.getenv("NOTIFY_AGENT_PORT", os.getenv("PORT", os.getenv("AGENT_PORT", "8202"))))
...
async def on_chat(ctx: Context, sender: str, msg: ChatMessage):
    ...
    await ctx.send(sender, ChatMessage(
        ...
        content=[TextContent(type="text", text=f"[notify] pong: {text or 'ping'}")]
    ))
...
@agent.on_message(model=Alert)
async def on_alert(ctx: Context, sender: str, alert: Alert):
    ctx.logger.info(f"[notify] ALERT [{alert.priority}] {alert.alert_type} | {alert.meta}")
